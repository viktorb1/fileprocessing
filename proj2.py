...
with open(save, 'w') as csvfile:
    ...
    for filename in args.json_file:
        if os.path.isfile("./sample-files/" + filename):
            ...
            row = [
                    data['timestamp'] if args.time_str_format is None else datetime.fromtimestamp(data['timestamp']).strftime(args.time_str_format),
                    ...
                    round(data['jobs'][0]['write']['bw_bytes'] / 1024, 1) if args.human_readable else data['jobs'][0]['write']['bw_bytes'],
                    data['jobs'][0]['error'],
                    filename
                ]
